...
from Tkinter import Frame, Label, Button, Menu, Toplevel, Entry
import time, sys
...
class FalconGUI:
    def __init__(self, master):
        ...
        self.tab_1 = Frame(self.master)
        self.tab_1.grid(row=0)
        self.monitor_1 = Label(self.tab_1, text='RX Monitor A')
        ...
        self.monitor_2 = Label(self.tab_1, text='RX Monitor B')
        self.monitor_2.pack(pady=10)
        self.table_2 = Frame(self.tab_1)
        self.table_2.pack(fill='both', padx=10)
    ...
    def data(self):
        ...
        for i in range(1, 9):
            ...
            link[i - 1] = Label(self.table_1, width=3, height=1)
            fail[i - 1] = Label(self.table_1, width=3, height=1, relief='solid', bd=1)
            if self.link_stat(i) == 1:
                ...
                fail[i - 1].config(text='FAIL', bg='red')
            eye[i - 1] = Label(self.table_1, text='%.3f' % self.eye_marg(i),
                               bg='white', width=3, height=1)
            ber[i - 1] = Label(self.table_1, text='%.2E' % self.ber(i),
                               bg='white', width=3, height=1)
        ...
        for i in range(1, 9):
            ...
            link[v - 1] = Label(self.table_2, width=3, height=1)
            fail[v - 1] = Label(self.table_2, width=3, height=1, relief='solid', bd=1)
            if self.link_stat(v) == 1:
                ...
                fail[v - 1].config(text='FAIL', bg='red')
            eye[v - 1] = Label(self.table_2, text='%.3f' % self.eye_marg(v),
                               bg='white', width=3, height=1)
            ber[v - 1] = Label(self.table_2, text='%.2E' % self.ber(v),
                               bg='white', width=3, height=1)
