...
class CredoNdUsbDongle:
	def __init__(self, phy_addr = 0, dev_addr = 1, shared_lib_path = ""):
		self.cr_phy_addr = phy_addr
		self.cr_dev_addr = dev_addr

		if platform.system() == 'Linux':
			shared_lib_name = "libfalcon_api.so"
		elif platform.system() == 'Windows':
			shared_lib_name = "falcon_api.dll"
		else:
...
class CredoUsbDongle:
	...
	def CredoUsbDongleOpen(self,usb_sel):
		ret = self.lib.cr_mdio_init(c.c_ubyte(self.cr_phy_addr),c.c_ubyte(self.cr_dev_addr),c.c_int(usb_sel))
		if ret != 0:
			ret = self.lib.cr_mdio_init(c.c_ubyte(self.cr_phy_addr),c.c_ubyte(self.cr_dev_addr),c.c_int(1))
			if ret != 0:
...
class Falcon_lib:
	def __init__(self):
		...
		#self.usb = CredoNdUsbDongle()
		#self.usb.CredoNdUsbDongleOpen()
		self.usb = CredoUsbDongle()
		self.macros = ['A0', 'A1', 'A2', 'A3']

	def connect(self,usb_sel = 0):
		port_index = self.usb.CredoUsbDongleOpen(usb_sel)
		return port_index
	...
	def DumpStateMachine(self, macro):
		...
		return [value_list,name_list,macro]
	...
	def LoadFirmware(self, path):
		self.usb.lib.LoadFirmware(c.c_char_p(path))
